...
macOS fell back to the blank generic document icon.

It now has one, generated by `tools/MakeIcon.swift` rather than drawn in an editor or
downsampled from a single master. The design is an amber panel dissolving into an
oversized transparency checkerboard: the visual language anyone who has opened Figma
or Photoshop reads instantly as "opacity".

**Every size is rendered from source, and each is a different drawing.** At 128 px
the tile carries six checker squares, a white rim and a drop shadow. At 16 px all
three are gone, leaving only the warm-to-cool split, because the checkerboard cannot
resolve below about 28 px and a 1 px rim around an 11 px tile is nearly a fifth of
its width, which bleached the amber out completely. That hinting is impossible if you
downsample one master, which is exactly why this is code and not a PNG.

The silhouette is a true superellipse rather than a rounded rectangle, sitting on
Apple's icon grid (824 units of a 1024 canvas).

Verified end to end rather than by eye alone: `AppIcon.icns` is present in the built
bundle at 47 KB where there was previously no icon resource, and
